...
#' @param dist Logical, if \code{TRUE} (default) then the midpoint of the first distance interval at which there is an insignificant difference is returned. If \code{FALSE} then the index of the first such interval is returned.
#' @param verbose Logical, if \code{TRUE} display results.
#' @return Midpoint of the smallest distance interval at which the observed distance distribution is insignificantly different from the null distribution.
