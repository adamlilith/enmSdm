...
#' @param rescale Numeric, value by which to multiply distances. For example, if distances are in meters, but it is visually more appealing to display them in kilometers, then \code{rescale} should equal 0.001 (km / m). Default is 1 (no re-scaling).
...
#' @seealso \code{\link[enmSdm]{spatialCorrForPoints}}, \code{\link[enmSdm]{spatialCorrForPointsSummary}}, \code{\link[enmSdm]{spatialCorrForPointsWeight}}
#' @examples
#' \dontrun{
...
spatialCorrForPointsPlot <- function(
	...
	# plot
	plot(midDists, midDists, ylim=c(0, ymax), col='white', xlab=xlab, ylab=ylab, ...)
	graphics::polygon(
	...
	graphics::lines(midDists, obs, type='b', pch=21, bg=bgs, ...)
