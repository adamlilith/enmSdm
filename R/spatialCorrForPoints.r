...
#' @param x Either a SpatialPoints or SpatialPointsDataFrame object, or a matrix or data frame with two columns where the first column has values for longitude and the second latitude.
...
#' @param fixed Either \code{NULL} (default) or a SpatialPoints or SpatialPointsDataFrame object, or a matrix or data frame with two columns where the first column has values for longitude and the second latitude. If not \code{NULL}, then the "observed" pairwise distance distribution is the set of pairwise distances between \code{x} and \code{fixed}. Only the points in \code{x} are randomly re-located.
...
spatialCorrForPoints <- function(
	...
	if (is.null(fixed)) {
		...
		obsDist <- geosphere::distm(x, fixed)
	}
