...
#' @param pts		Either a SpatVector, SpatialPoints, or SpatialPointsDataFrame object, or a matrix or data frame with two columns where the first column has values for longitude and the second latitude.
#' @param rast		Either a SpatRster, raster, raster stack, or raster brick. Randomly located sites will be placed in any non-\code{NA} cell on this raster. If this is a raster stack or brick, the first layer will be used.
...
#' The function \code{spatialCorrForPointsWeight} calculates weights for a set of points based on the characteristic scale of spatial autocorrelation.  \cr
#'
#' @return A data frame. The first three columns represent the lower boundary, middle, and upper boundary for each distance bin. The column named \code{observedProportion} represents the proportion of observed pairwise distances in each bin. Subsequent columns are named \code{randProportion} (one per randomization iteration) which are the proportion of pairwise distances from randomized points that fall into each bin.
#'
...
spatialCorrForPoints <- function(
	...
	# convert
	if (inherits(pts, 'SpatVector')) pts <- as(pts, 'Spatial')
	...
	for (iter in 1:iters) {
		...
		if (is.null(fixed)) {
			...
			randDist <- geosphere::distm(randPoints, fixed)
		}
