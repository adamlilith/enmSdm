...
#' @param cores Integer >= 0, number of cores to use (can speed large computations).
#' @param na.rm Logical, if \code{TRUE} then any rows in \code{set1} or \code{set2} with at least one \code{NA} are removed first.
#' @return List with three elements. The first two elements correspond to \code{set1} and \code{set2}. Each of these elements is a list the same length of \code{p}, with each data frame/matrix corresponding to a value of \code{p}. The third element is a matrix of statistics reporting the statistics pertaining to the environmental distances between each subset of \code{set1} and \code{set2}.
...
mop <- function(
	...
	if (cores == 1) {
		...
		# parallel::clusterEvalQ(cl=clust, library(omnibus))

		x <- lapply(parallel::splitIndices(nrow(set1), n), function(i) set1[i, , drop = FALSE])
		dists <- parallel::parLapply(clust, X=x, fun=omnibus::pairDist, set2=set2)
