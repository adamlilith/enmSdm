#' Thin geographic points such that none are within a minimum distance of any other
#'
...
#' @param minDist Numeric. Minimum distance needed between points to retain them. Points falling < this distance will be discarded. if \code{distFunct} is \code{distCosine} then this should be in thesame units as \code{f} (see [geosphere::distCosine()] and related "\code{dist}" functions).
