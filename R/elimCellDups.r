...
#' r <- raster::raster() # empty raster covering entire world with 1-degree resolution
#' elimCellDups(x, r, longLat=c(1, 2))
#' elimCellDups(x, r, longLat=c(1, 2), priority=c(3, 2, 1, 0))
#' @export
...
elimCellDups <- function(
	...
	if (is(x, 'SpatialPoints') | is(x, 'SpatialPointsDataFrame')) {
	...
	if (length(removeThese) > 0) {
		x <- if (is(x, 'matrix') | is(x, 'data.frame') | is(x, 'SpatialPointsDataFrame')) {
			x[-removeThese, ]
		} else {
			x[-removeThese]
		}
