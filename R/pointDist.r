...
#'
#' @param x A data frame, matrix, \code{SpatialPoints}, \code{SpatialPointsDataFrame}, or \code{SpatVector} object. If a data frame or matrix then the coordinate reference system is assumed to be unprojected (WGS84). Note that if you are using \pkg{terra} \code{SpatVector} objects, then the \code{distance} function in that package will be much faster for large sets of points.
#' @param y If \code{NULL} then pairwise distances are calculated between all points in \code{x}. However, if \code{y} is a data frame, matrix, \code{SpatialPoints}, \code{SpatialPointsDataFrame}, or \code{SpatVector} object then distances are between all points in \code{x} and \code{y}.
...
#' @seealso \code{\link[geosphere]{distCosine}}, \code{\link[omnibus]{pairDist}}, \code{\link[terra]{distance}}
#' @examples
#'
#' set.seed(17)
...
pointDist <- function(x, y = NULL, distFunct = NULL, longLat = NULL, ...) {
	...
	# get coordinates
	x <- xToCoords(x, longLat=longLat, out='sp')
	if (is.null(y)) {
		y <- x
	} else {
		y <- xToCoords(y, longLat=longLat, out='sp')
	}
